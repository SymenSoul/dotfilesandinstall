...
class MdFileRenameProvider {
    ...
    async #addDirectoryRenameEdits(edit, builder, token) {
        ...
        for (const [docUri, links] of allLinksInWorkspace) {
            for (const link of links) {
                if (link.href.kind !== documentLink_1.HrefKind.Internal || link.source.hrefText.startsWith('#')) {
                    continue;
                }
                ...
                if (link.source.pathText.startsWith('..') && (0, path_1.isParentDir)(edit.newUri, docUri)) {
                    ...
                    const oldLink = (0, mdLinks_1.resolveInternalDocumentLink)(oldDocUri, link.source.hrefText, this.#workspace);
                    if (oldLink) {
                        let newPathText;
                        ...
                        if (replacementPath !== link.source.pathText) {
                            const { range, newText } = (0, rename_1.getLinkRenameEdit)(link, replacementPath);
    ...
    async #tryAddEditsInSelf(edit, allEdits, builder) {
        if (!(0, path_1.looksLikeMarkdownUri)(this.#config, edit.newUri)) {
            return false;
        }
    ...
    async #addEditsForLinksInSelf(doc, link, edit, allEdits, builder) {
        ...
        // Resolve the link relative to the old file path
        let oldLink = (0, mdLinks_1.resolveInternalDocumentLink)(edit.oldUri, link.source.hrefText, this.#workspace);
        if (!oldLink) {
            return false;
        }
        // See if the old link was effected by one of the renames
        for (const edit of allEdits) {
            if ((0, path_1.isSameResource)(edit.oldUri, oldLink.resource) || (0, path_1.isParentDir)(edit.oldUri, oldLink.resource)) {
    ...
    #addLinkRenameEdit(doc, link, newUri, builder) {
        ...
        const newFilePath = (0, mdLinks_1.removeNewUriExtIfNeeded)(this.#config, link.href, newUri);
        ...
        if (typeof newLinkText === 'string') {
            const { range, newText } = (0, rename_1.getLinkRenameEdit)(link, newLinkText);
