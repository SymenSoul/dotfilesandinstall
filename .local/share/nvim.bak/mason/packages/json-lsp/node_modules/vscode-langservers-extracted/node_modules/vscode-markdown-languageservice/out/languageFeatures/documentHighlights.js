...
class MdDocumentHighlightProvider {
    ...
    *#getHighlightsForHeader(document, header, links, toc) {
        ...
        for (const link of links) {
            if (link.href.kind === documentLink_1.HrefKind.Internal
                ...
                && (0, path_1.isSameResource)(link.href.path, docUri)) {
                yield {
    ...
    #getHighlightsForLinkAtPosition(document, position, links, toc) {
        const link = links.find(link => (0, range_1.rangeContains)(link.source.hrefRange, position) || (link.kind === documentLink_1.MdLinkKind.Definition && (0, range_1.rangeContains)(link.ref.range, position)));
        if (!link) {
            return [];
        }
    ...
    *#getHighlightsForLinkFragment(document, href, links, toc) {
        ...
        if ((0, path_1.isSameResource)(targetDoc, (0, textDocument_1.getDocUri)(document))) {
        ...
        for (const link of links) {
            if (link.href.kind === documentLink_1.HrefKind.Internal && (0, path_1.looksLikePathToResource)(this.#configuration, link.href.path, targetDoc)) {
    ...
    *#getHighlightsForExternalLink(uri, links) {
        for (const link of links) {
            if (link.href.kind === documentLink_1.HrefKind.External && (0, path_1.isSameResource)(link.href.uri, uri)) {
                yield {
                    range: (0, rename_1.getFilePathRange)(link),
                    kind: lsp.DocumentHighlightKind.Read,
                };
    ...
    *#getHighlightsForReference(ref, links) {
        for (const link of links) {
            if (link.kind === documentLink_1.MdLinkKind.Definition && link.ref.text === ref) {
                yield {
                    range: link.ref.range,
                    kind: lsp.DocumentHighlightKind.Write,
                };
            }
            else if (link.href.kind === documentLink_1.HrefKind.Reference && link.href.ref === ref) {
                yield {
