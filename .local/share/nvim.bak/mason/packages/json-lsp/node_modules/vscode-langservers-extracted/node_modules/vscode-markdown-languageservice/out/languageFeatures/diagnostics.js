...
class DiagnosticComputer {
    ...
    async compute(doc, options, token) {
        ...
        this.#logger.log(logging_1.LogLevel.Trace, 'DiagnosticComputer.compute finished', { document: doc.uri, version: doc.version, diagnostics });
        return {
            links: links,
            statCache,
    ...
    async #validateFragmentLinks(doc, options, links, token) {
        ...
        for (const link of links) {
            if (link.href.kind === documentLink_1.HrefKind.Internal
                ...
                && (0, path_1.isSameResource)(link.href.path, (0, textDocument_1.getDocUri)(doc))
                && link.href.fragment
                && !toc.lookup(link.href.fragment)) {
                // Don't validate line number links
                if ((0, path_1.parseLocationInfoFromFragment)(link.href.fragment)) {
                    continue;
                }
    ...
    *#validateReferenceLinks(options, links, definitions) {
        ...
        for (const link of links) {
            if (link.href.kind === documentLink_1.HrefKind.Reference && !definitions.lookup(link.href.ref)) {
                yield {
    ...
    *#validateUnusedLinkDefinitions(options, links) {
        ...
        const usedRefs = new documentLink_1.ReferenceLinkMap();
        for (const link of links) {
            if (link.kind === documentLink_1.MdLinkKind.Link && link.href.kind === documentLink_1.HrefKind.Reference) {
                usedRefs.set(link.href.ref, true);
            }
        }
        for (const link of links) {
            if (link.kind === documentLink_1.MdLinkKind.Definition && !usedRefs.lookup(link.ref.text)) {
                yield {
                ...
                if (!resolvedHrefPath) {
                    ...
                    if (fragmentLinks.length) {
                        ...
                        for (const link of fragmentLinks) {
                            // Don't validate line number links
                            if ((0, path_1.parseLocationInfoFromFragment)(link.fragment)) {
                                continue;
                            }
...
class FileLinkState extends dispose_1.Disposable {
    #onDidChangeLinkedToFile = this._register(new lsp.Emitter);
    /**
    ...
    updateLinksForDocument(document, links, statCache) {
        const linkedToResource = new Set(links
            .filter(link => link.href.kind === documentLink_1.HrefKind.Internal)
...
class DiagnosticsManager extends dispose_1.Disposable {
    #computer;
    #linkWatcher;
    #onLinkedToFileChanged = this._register(new lsp.Emitter());
