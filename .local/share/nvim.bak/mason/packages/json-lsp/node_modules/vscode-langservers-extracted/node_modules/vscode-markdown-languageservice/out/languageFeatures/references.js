...
})(MdReferenceKind || (exports.MdReferenceKind = MdReferenceKind = {}));
/**
...
class MdReferencesProvider extends dispose_1.Disposable {
    ...
    async #getReferencesToHeader(document, header, token) {
        ...
        for (const link of links) {
            if (link.href.kind === documentLink_1.HrefKind.Internal
                && (0, path_1.looksLikePathToResource)(this.#configuration, link.href.path, (0, textDocument_1.getDocUri)(document))
    ...
    async #getReferencesToLink(docLinks, sourceLink, triggerPosition, token) {
        ...
        if (sourceLink.href.kind === documentLink_1.HrefKind.External) {
            const references = [];
            for (const link of allLinksInWorkspace) {
                if (link.href.kind === documentLink_1.HrefKind.External && (0, path_1.isSameResource)(link.href.uri, sourceLink.href.uri)) {
        ...
        if (resolvedResource && this.#isMarkdownPath(resolvedResource) && sourceLink.href.fragment && sourceLink.source.fragmentRange && (0, range_1.rangeContains)(sourceLink.source.fragmentRange, triggerPosition)) {
            ...
            for (const link of allLinksInWorkspace) {
                if (link.href.kind !== documentLink_1.HrefKind.Internal || !(0, path_1.looksLikePathToResource)(this.#configuration, link.href.path, resolvedResource)) {
                    continue;
                }
    ...
    #isMarkdownPath(resolvedHrefPath) {
        return this.#workspace.hasMarkdownDocument(resolvedHrefPath) || (0, path_1.looksLikeMarkdownUri)(this.#configuration, resolvedHrefPath);
    }
    *#findLinksToFile(resource, links, sourceLink) {
        for (const link of links) {
            if (link.href.kind !== documentLink_1.HrefKind.Internal || !(0, path_1.looksLikePathToResource)(this.#configuration, link.href.path, resource)) {
                continue;
            }
    ...
    *#getReferencesToLinkReference(allLinks, refToFind, from) {
        for (const link of allLinks) {
            ...
            else if (link.href.kind === documentLink_1.HrefKind.Reference) {
                ref = link.href.ref;
            }
            else {
                continue;
            ...
            if (ref === refToFind && link.source.resource.fsPath === from.resource.fsPath) {
                const isTriggerLocation = from.resource.fsPath === link.source.resource.fsPath && ((link.href.kind === documentLink_1.HrefKind.Reference && (0, range_1.areRangesEqual)(from.range, link.source.hrefRange)) || (link.kind === documentLink_1.MdLinkKind.Definition && (0, range_1.areRangesEqual)(from.range, link.ref.range)));
                const pathRange = this.#getPathRange(link);
                yield {
                    kind: MdReferenceKind.Link,
                    isTriggerLocation,
                    isDefinition: link.kind === documentLink_1.MdLinkKind.Definition,
                    link,
